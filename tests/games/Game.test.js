...
describe('Game base class', () => {
  ...
  describe('abstract methods', () => {
    it('base Game class throws error on update', () => {
      ...
      expect(() => baseGame.update(16)).toThrow('update() must be implemented by subclass')
    })

    it('base Game class throws error on render', () => {
      const baseGame = new Game(canvas, input, audio)
      expect(() => baseGame.render(game.ctx)).toThrow('render() must be implemented by subclass')
    })
