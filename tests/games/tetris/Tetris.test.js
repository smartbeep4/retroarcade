...
describe('Tetris', () => {
  ...
  describe('piece spawning', () => {
    ...
    it('spawns pieces at top center', () => {
      game.spawnPiece()
      const expectedX = Math.floor((game.cols - game.currentPiece.shape[0].length) / 2)
  ...
  describe('collision detection', () => {
    it('detects left wall collision', () => {
      const collision = game.checkCollisionWithShape(game.currentPiece.shape, -1, 0)
      expect(collision).toBe(true)
    })

    it('detects right wall collision', () => {
      const collision = game.checkCollisionWithShape(game.currentPiece.shape, game.cols, 0)
      expect(collision).toBe(true)
    })

    it('detects floor collision', () => {
      const collision = game.checkCollisionWithShape(game.currentPiece.shape, 0, game.rows)
      expect(collision).toBe(true)
    })
  ...
  describe('rendering', () => {
    ...
    it('draws hold piece in UI', () => {
      game.holdPiece = 'I'
      game.render(mockCtx)
      expect(mockCtx.fillText).toHaveBeenCalledWith('HOLD', expect.any(Number), expect.any(Number))
    })

    it('draws next pieces in UI', () => {
      game.render(mockCtx)
      expect(mockCtx.fillText).toHaveBeenCalledWith('NEXT', expect.any(Number), expect.any(Number))
    })

    it('draws score in UI', () => {
      game.score = 1000
      game.render(mockCtx)
      expect(mockCtx.fillText).toHaveBeenCalledWith('SCORE', expect.any(Number), expect.any(Number))
      expect(mockCtx.fillText).toHaveBeenCalledWith('1000', expect.any(Number), expect.any(Number))
    })

    it('draws level in UI', () => {
      game.level = 5
      game.render(mockCtx)
      expect(mockCtx.fillText).toHaveBeenCalledWith('LEVEL', expect.any(Number), expect.any(Number))
    ...
    it('draws lines cleared in UI', () => {
      ...
      game.render(mockCtx)
      expect(mockCtx.fillText).toHaveBeenCalledWith('LINES', expect.any(Number), expect.any(Number))
      expect(mockCtx.fillText).toHaveBeenCalledWith('42', expect.any(Number), expect.any(Number))
    })
  })
