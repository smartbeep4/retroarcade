...
export class SnakePage extends GamePage {
   ...
   */
  async verifyDirectionChange(newDirection: 'up' | 'down' | 'left' | 'right'): Promise<boolean> {
    ...
    const is180Turn = currentDir.x + newDir.x === 0 && currentDir.y + newDir.y === 0
    return !is180Turn
  }
  ...
  async hasSnakeHeadOnCanvas(): Promise<boolean> {
    ...
    return await this.canvas.hasColorInRegion(this.SNAKE_HEAD_COLOR, 0, 0, dims.width, dims.height)
  }
  ...
  async hasFoodOnCanvas(): Promise<boolean> {
    const dims = await this.canvas.getCanvasDimensions()
    return await this.canvas.hasColorInRegion(this.FOOD_COLOR, 0, 0, dims.width, dims.height)
  }
}
