...
test.describe("Accessibility Tests", () => {
  test.describe("Main Menu", () => {
    test("main menu should have no critical accessibility violations", async ({
      page,
    }) => {
  ...
  test.describe("Game States", () => {
    ...
    for (const { id, name } of games) {
      test(`${name} game page should have no critical a11y violations`, async ({
        page,
      }) => {
        const arcade = new ArcadeShellPage(page);
        await arcade.goto();
  ...
  test.describe("Focus Management", () => {
    test("focus should be visible during keyboard navigation", async ({
      page,
    }) => {
      const arcade = new ArcadeShellPage(page);
      await arcade.goto();
