...
test.describe('Visual Regression Tests', () => {
  test.describe('Main Menu', () => {
    test('main menu should match baseline', async ({ page }) => {
      ...
      await expect(page.locator('#game-canvas')).toHaveScreenshot('main-menu.png', {
    ...
    test('menu navigation highlight should work', async ({ page }) => {
      ...
      await expect(page.locator('#game-canvas')).toHaveScreenshot('menu-selection-pong.png', {
  ...
  test.describe('Game Initial States', () => {
    ...
    for (const { id, name } of games) {
      test(`${name} initial state should match baseline`, async ({ page }) => {
        ...
        await expect(page.locator('#game-canvas')).toHaveScreenshot(`game-${id}-initial.png`, {
  ...
  test.describe('Pause Menu', () => {
    test('pause menu should match baseline', async ({ page }) => {
      ...
      await expect(page.locator('#game-canvas')).toHaveScreenshot('pause-menu.png', {
        maxDiffPixelRatio: 0.05,
      })
    })
...
test.describe('Game Color Validation', () => {
  test('Snake should use correct colors', async ({ page }) => {
    ...
    const hasSnakeHead = await arcade.canvasInspector.hasColorInRegion('#39ff14', 0, 0, 800, 600)
    expect(hasSnakeHead).toBe(true)

    // Check for food color (pink/red)
    const hasFood = await arcade.canvasInspector.hasColorInRegion('#ff2a6d', 0, 0, 800, 600)
  ...
  test('Menu should use cyan theme color', async ({ page }) => {
    ...
    const hasCyan = await arcade.canvasInspector.hasColorInRegion('#05d9e8', 0, 0, 800, 600)
