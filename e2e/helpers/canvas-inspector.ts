...
export class CanvasInspector {
   ...
   */
  async countPixelsOfColor(color: string, tolerance: number = 10): Promise<number> {
    const dims = await this.getCanvasDimensions()
    const matches = await this.findColorInRegion(color, 0, 0, dims.width, dims.height, tolerance)
